...
const targetPath = 'app/store/[slug]/page.tsx';
...
const oldFilter = "const matchesCategory = selectedCategory === 'all' || p.category === selectedCategory;";
const newFilter = "const matchesCategory = selectedCategory === 'all' || p.category === selectedCategory || p.category.startsWith(selectedCategory + ' > ');";
content = content.replace(oldFilter, newFilter);

// 3. Replace features_strip with dynamic categories bar
const featuresStripRegex = /\/\/ --- SECTION: FEATURES STRIP ---[\s\S]*?if \(section\.type === 'features_strip'\) \{[\s\S]*?\}\n/g;

const dynamicCategoriesBar = `
...
content = content.replace(featuresStripRegex, dynamicCategoriesBar);

// 4. Remove old Category Pills
const oldPillsRegex = /\{\/\* Category Pills \*\/\}.*?\{categories\.map[\s\S]*?\)\)\}\n\s*<\/div>/g;
content = content.replace(oldPillsRegex, "");
